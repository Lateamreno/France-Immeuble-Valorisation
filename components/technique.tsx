...
import { pictoComposant } from "@/lib/pictos-composants";
 ...
 */
// Les dessins vivent dans lib/pictos-composants.tsx : ils servent aussi au
// dossier de vente, qui est un composant serveur (un module « use client » ne
// peut pas lui prêter une constante).
...
function ComposantsTab({ b }: { b: BienData }) {
  ...
  const depart = () =>
    Object.fromEntries(affichees.map(({ cle, composant }) => [
      cle,
      {
        materiau: S(composant?.["Type_matériau"]) === "Autre"
          ? (S(composant?.["type_matériau_autre"]) || "Autre")
          : S(composant?.["Type_matériau"]),
        etat: S(composant?.Etat),
      } as Ligne,
    ]));
